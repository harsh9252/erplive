...
(function () {
	...
	// Mobile menu sidebar overlay - updated to work with React
	// Check if sidebar overlay already exists, if not create it
	if ($('.sidebar-overlay').length === 0) {
		$('body').append('<div class="sidebar-overlay"></div>');
	}

	// Handle mobile button click - using event delegation for React compatibility
	...
	// Datatable - Initialize with delay to allow React to render
	function initDataTables() {
		if($('.datatable').length > 0) {
			$('.datatable').each(function() {
				...
				// Skip if table is empty or has no rows (React might still be rendering)
				if ($(this).find('tbody tr').length === 0) {
					return;
				}
				
				// Check if table contains React Router links (they'll have data-reactroot or similar)
				var hasReactContent = $(this).find('a[href^="#"]').length > 0 || 
									  $(this).closest('[data-reactroot]').length > 0 ||
									  $(this).html().includes('Link');
				
				if (hasReactContent) {
					console.log('Skipping DataTable initialization for React-managed table');
					return;
				}
				...
				try {
					...
					console.warn('DataTables initialization failed for table:', error.message);
				}
			});
		}
	}
...
// select all 2
	...
	$(".add-invoice-data").on('click',function(e){
		...
		var addTable = `<tr>
							...
							<td>
								<div>
									<Link href="javascript:void(0);" class="text-danger remove-table"><i class="isax isax-close-circle"></i></a>
								</div>
							</td>
						</tr>`; 
