...
const DebitNoteDetails = () => {
  ...
  const handlePost = async () => {
    const { isConfirmed } = await Swal.fire({
      ...
      text: "Are you sure you want to post this debit note? This will reduce stock (OUT) from the warehouse.",
      icon: 'question',
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div>
          <h4 className="fw-bold mb-1">Debit Note: {dn.debit_note_number || dn.debit_number || dn.invoice_number || dn.id}</h4>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb mb-0 fs-13">
              <li className="breadcrumb-item"><Link to="/invoicing/debit-notes">Debit Notes</Link></li>
              <li className="breadcrumb-item active">{dn.debit_note_number || dn.debit_number || dn.invoice_number}</li>
            </ol>
          </nav>
        </div>
            ...
            <div className="card-body p-4">
              ...
              <div className="row g-3 mb-5 py-3 bg-light rounded-12">
                <div className="col-sm-3 col-6 border-end">
                  ...
                  <span className="fw-bold text-dark">{dn.debit_note_date || dn.debit_date || dn.date}</span>
                </div>
                <div className="col-sm-3 col-6 border-end">
                  <span className="text-muted fs-12 d-block mb-1">Linked Purchase</span>
                  <span className="fw-bold text-primary">{dn.original_invoice_number || dn.purchase_invoice_id ? 'Invoice' : 'Direct'}</span>
                </div>
                ...
                <div className="col-sm-3 col-6 text-end px-3">
                  ...
                  <span className="fw-bold text-primary h5 mb-0">₹{(dn.net_total || dn.net_amount || 0).toLocaleString()}</span>
                </div>
              </div>
          ...
          <div className="card border-0 shadow-sm mb-4">
            ...
            <div className="card-body">
              <div className="d-flex justify-content-between mb-2 fs-14">
                ...
                <span className="fw-bold">₹{Number(dn.taxable_amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              {isInterState ? (
                <div className="d-flex justify-content-between mb-2 fs-14">
                  <span className="text-muted">IGST Total</span>
                  <span className="fw-bold">₹{Number(dn.igst || dn.tax_amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
              ) : (
                <>
                  <div className="d-flex justify-content-between mb-2 fs-14">
                    <span className="text-muted">CGST Total</span>
                    <span className="fw-bold">₹{Number(dn.cgst || (dn.tax_amount / 2) || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="d-flex justify-content-between mb-2 fs-14">
                    <span className="text-muted">SGST Total</span>
                    <span className="fw-bold">₹{Number(dn.sgst || (dn.tax_amount / 2) || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                </>
              )}
              ...
              <div className="d-flex justify-content-between align-items-center">
                ...
                <span className="h4 fw-bold text-primary mb-0">₹{(dn.net_total || dn.net_amount || 0).toLocaleString()}</span>
              </div>
            </div>
          </div>
