...
import { getTcsSummaryReport } from '../services/reportService';
...
const TcsReport = () => {
  ...
  const fetchReportData = async (params = {}) => {
    try {
      ...
      const response = await getTcsSummaryReport(finalParams);
      ...
      const mappedItems = items.map(item => ({
        ...
        section: item.section || item.tcs_section || '-',
        base_amount: item.base_amount || item.taxable_amount || 0,
        rate: item.rate || item.tcs_rate || 0,
        tcs_amount: item.tcs_amount || item.tax_amount || 0
      }));
      ...
      console.error("Error fetching TCS summary:", err);
      setError("Failed to load TCS report.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div>
          <h4 className="fw-bold mb-1">TCS Report</h4>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb mb-0 fs-13">
              <li className="breadcrumb-item"><Link to="/">Home</Link></li>
              <li className="breadcrumb-item">Tax & Compliances</li>
              <li className="breadcrumb-item active">TCS Report</li>
            </ol>
          </nav>
        </div>
      ...
      <div className="row g-3 mb-4">
        {[
          { label: 'Total TCS Collected', amount: `₹${data.reduce((sum, i) => sum + (Number(i.tcs_amount) || 0), 0).toLocaleString()}`, icon: 'wallet-money', color: 'primary' },
          { label: 'Total Base Amount', amount: `₹${data.reduce((sum, i) => sum + (Number(i.base_amount) || 0), 0).toLocaleString()}`, icon: 'money-3', color: 'success' },
          { label: 'Average TCS Rate', amount: `${data.length ? (data.reduce((sum, i) => sum + (Number(i.rate) || 0), 0) / data.length).toFixed(2) : 0}%`, icon: 'percentage', color: 'warning' },
            ...
            <table className="table table-hover align-middle mb-0">
              <thead className="bg-light">
                <tr>
                  ...
                  {columns.tcs_amount && <th className="cursor-pointer text-end pe-4" onClick={() => requestSort('tcs_amount')}>TCS Amount {sortConfig.key === 'tcs_amount' && (sortConfig.direction === 'asc' ? '↑' : '↓')}</th>}
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="8" className="text-center py-5">
                      ...
                      <span className="text-muted">Loading TCS report...</span>
                    </td>
                  </tr>
                ) : error ? (
                  <tr>
                  ...
                  filteredData.map((item, index) => (
                    <tr key={item.id || index}>
                      ...
                      {columns.section && <td><span className="badge badge-soft-warning">{item.section}</span></td>}
                      ...
                      {columns.tcs_amount && <td className="text-end fw-bold text-dark pe-4">₹{(item.tcs_amount || 0).toLocaleString()}</td>}
                    </tr>
                  ))
                )}
              </tbody>
