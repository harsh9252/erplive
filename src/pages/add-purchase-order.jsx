...
const AddPurchaseOrder = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setVendors(Array.isArray(venRes.data) ? venRes.data : (venRes.data?.rows || []));
  ...
  const addItemRow = () => {
    setFormData(prev => ({
      ...
      items: [
        ...prev.items,
        { item_id: '', description: '', quantity: 1, rate: 0 }
      ]
    }));
  };
  ...
  return (
    <div className="container-fluid py-4 text-dark">
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div>
          <h4 className="fw-bold mb-1">{isEditMode ? 'Edit' : 'Create'} Purchase Order</h4>
          ...
          <div className="card-body">
            <div className="row g-3">
              <div className="col-md-4">
                ...
                <select className="form-select shadow-none" name="vendor_id" value={formData.vendor_id} onChange={handleHeaderChange} required>
              ...
              <div className="col-md-4">
                <label className="form-label fw-600">PO Number <span className="text-danger">*</span></label>
                <input type="text" className="form-control shadow-none" name="po_number" value={formData.po_number} onChange={handleHeaderChange} placeholder="e.g. PO-2026-001" required />
              </div>
              ...
              <table className="table align-middle mb-0">
                ...
                <tbody>
                  {formData.items.map((item, index) => (
                    <tr key={index}>
                      <td>
                        <select className="form-select shadow-none border-0 bg-transparent mb-1" value={item.item_id} onChange={(e) => handleItemChange(index, 'item_id', e.target.value)} required>
                          <option value="">Select Item</option>
                          {items.map(i => (
                            <option key={i.id} value={i.id}>{i.name}</option>
                          ))}
                        </select>
                        <input type="text" className="form-control shadow-none border-0 bg-transparent fs-12 text-muted" value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} placeholder="Additional description..." />
                      </td>
                      <td>
                        <input type="number" className="form-control shadow-none border-0 bg-transparent text-center" value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', e.target.value)} min="0.1" step="any" required />
                      </td>
                      <td>
          ...
          <div className="col-lg-7">
            <div className="card border-0 shadow-sm h-100">
              <div className="card-body">
                ...
                <textarea className="form-control shadow-none mb-3" name="notes" value={formData.notes} onChange={handleHeaderChange} rows="3" placeholder="Enter notes to vendor..."></textarea>
                <label className="form-label fw-600">Internal Remarks</label>
                <textarea className="form-control shadow-none" name="remarks" value={formData.remarks} onChange={handleHeaderChange} rows="2" placeholder="Private notes..."></textarea>
              </div>
            </div>
          </div>
            ...
            <div className="card border-0 shadow-sm bg-primary bg-opacity-10 h-100">
              <div className="card-body">
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                  ...
                  <span className="h4 fw-bold mb-0 text-primary">₹{summary.netTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="row g-2">
                  <div className="col-6">
                    <button type="button" className="btn btn-outline-white w-100 rounded-pill border py-2 shadow-none" onClick={() => navigate('/invoicing/purchase-orders')}>Cancel</button>
                  </div>
                  <div className="col-6">
