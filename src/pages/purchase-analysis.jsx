...
const PurchaseAnalysis = () => {
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      utils.book_append_sheet(wb, ws, "Purchase Analysis");
      writeFile(wb, `Purchase_Analysis.xlsx`);
    } catch (error) {
      console.error(error);
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const doc = new jsPDF();
      doc.setFontSize(16);
      doc.text('Purchase Analysis', 14, 15);
      doc.setFontSize(10);
      doc.text(`Period: ${filters.from_date} to ${filters.to_date}`, 14, 22);

      const tableColumns = [columns[0].label, "Purchase Amount", "Bill Count", "Avg Value"];
      ...
      doc.save(`Purchase_Analysis.pdf`);
    } catch (error) {
      console.error(error);
