...
import { getSalaryStructure, saveSalaryStructure } from '../services/salaryStructureService';
...
const SalaryStructure = () => {
            ...
            try {
                ...
                if (data && Object.keys(data).length > 0) {
                    setFormData({
                        ...
                        effective_from: data.effective_from ? data.effective_from.split('T')[0] : new Date().toISOString().split('T')[0]
                    });
                } else {
                    // Reset to defaults if no structure found
                    setFormData({
                        basic: 0,
                        hra: 0,
                        special_allowance: 0,
    ...
    // Calculation Logic
    useEffect(() => {
        const { basic, hra, special_allowance, other_allowance, medical_allowance, transport_allowance } = formData;
        ...
        const ta = parseFloat(transport_allowance) || 0;

        const gross = b + h + sa + oa + ma + ta;
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            const msg = error.response?.data?.message || 'Failed to update salary structure';
            toast.error(msg);
        } finally {
            setLoading(false);
        }
        ...
        <div className="container-fluid py-4">
            ...
            <div className="row g-4">
                ...
                <div className="col-md-4">
                    <div className="card border-0 shadow-sm h-100">
                        ...
                        <div className="card-body">
                            <div className="mb-3">
                                <label className="form-label small fw-bold text-muted">Employee Search</label>
                                <SearchableSelect 
                                    options={employees}
                            ...
                            <div className="mt-4 pt-4 border-top">
                                <h6 className="fw-bold mb-3 small text-uppercase text-primary">Calculation Summary</h6>
                                <div className="d-flex flex-column gap-3">
                                    <div className="d-flex justify-content-between align-items-center">
                                        <span className="text-muted small">Gross Earnings</span>
                                        <span className="fw-bold text-dark">₹{calculations.gross.toLocaleString()}</span>
                                    </div>
                                    <div className="d-flex justify-content-between align-items-center text-danger">
                                        <span className="small">Total Deductions</span>
                                        <span className="fw-bold">₹{(calculations.pf_employee + calculations.esi_employee + calculations.pt).toLocaleString()}</span>
                                    </div>
                                    <div className="p-3 bg-primary bg-opacity-10 border border-primary border-opacity-25 rounded mt-2">
                                        <div className="d-flex justify-content-between align-items-center">
                                            <span className="text-primary fw-bold small">Net Payable</span>
                                            <span className="text-primary h5 fw-bold mb-0">₹{calculations.net_salary.toLocaleString()}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-4 p-3 bg-light rounded small">
                                <p className="mb-0 text-muted italic">Note: Deductions are auto-calculated based on statutory rules. Net Monthly Salary = Total Earnings - Statutory Deductions.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="col-md-8">
                    <form onSubmit={handleSave} className="card border-0 shadow-sm">
                        ...
                        <div className="card-body p-4">
                            <div className="row g-4">
                                <div className="col-md-6">
                                    <label className="form-label small fw-bold text-muted">Effective From *</label>
                                    <input 
                                        type="date" 
                                        name="effective_from"
                                ...
                                <div className="col-md-6">
                                    <label className="form-label small fw-bold text-muted">HRA (House Rent Allowance)</label>
                                    <div className="input-group">
                                        <span className="input-group-text bg-light border-end-0">₹</span>
                                        <input 
                                            type="number" 
                                ...
                                <div className="col-md-4">
                                    <label className="form-label small fw-bold text-muted">Special Allowance</label>
                                    <div className="input-group">
                                        <span className="input-group-text bg-light border-end-0">₹</span>
                                        <input 
                                            type="number" 
                                ...
                                <div className="col-md-4">
                                    <label className="form-label small fw-bold text-muted">Other Allowance</label>
                                    <div className="input-group">
                                        <span className="input-group-text bg-light border-end-0">₹</span>
                                        <input 
                                            type="number" 
                                ...
                                <div className="col-md-4">
                                    <label className="form-label small fw-bold text-muted">Medical Allowance</label>
                                    <div className="input-group">
                                        <span className="input-group-text bg-light border-end-0">₹</span>
                                        <input 
                                            type="number" 
                                ...
                                <div className="col-md-4">
                                    <label className="form-label small fw-bold text-muted">Transport Allowance</label>
                                    <div className="input-group">
                                        <span className="input-group-text bg-light border-end-0">₹</span>
                                        <input 
                                            type="number" 
                                    ...
                                    <div className="col-md-6">
                                        <div className="p-3 bg-light rounded border border-dashed">
                                            <div className="d-flex justify-content-between align-items-center mb-1">
                                                <span className="small text-muted">Provident Fund (PF)</span>
                                                <span className="fw-bold">₹{calculations.pf_employee}</span>
                                            </div>
                                            <div className="progress" style={{ height: '4px' }}>
                                                <div className="progress-bar bg-danger" style={{ width: calculations.gross > 0 ? (calculations.pf_employee/calculations.gross*100) + '%' : '0%' }}></div>
                                            </div>
                                            <p className="mb-0 mt-2 fs-11 text-muted italic">12% match on Basic (Capped at ₹1,800)</p>
                                        </div>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="p-3 bg-light rounded border border-dashed">
                                            <div className="d-flex justify-content-between align-items-center mb-1">
                                                ...
                                                <span className="fw-bold">₹{calculations.esi_employee}</span>
                                            </div>
                                            <div className="progress" style={{ height: '4px' }}>
                                                <div className="progress-bar bg-danger" style={{ width: calculations.gross > 0 ? (calculations.esi_employee/calculations.gross*100) + '%' : '0%' }}></div>
                                            </div>
                                            <p className="mb-0 mt-2 fs-11 text-muted italic">0.75% of Gross (Applicable if Gross ≤ ₹21k)</p>
                                        </div>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="p-3 bg-light rounded border border-dashed">
                                            <div className="d-flex justify-content-between align-items-center mb-1">
                                                <span className="small text-muted">Professional Tax (PT)</span>
                                                <span className="fw-bold">₹{calculations.pt}</span>
                                            </div>
                                            <div className="progress" style={{ height: '4px' }}>
                                                <div className="progress-bar bg-danger" style={{ width: calculations.gross > 0 ? (calculations.pt/calculations.gross*100) + '%' : '0%' }}></div>
                                            </div>
                                            <p className="mb-0 mt-2 fs-11 text-muted italic">Tiered based on monthly salary threshold</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
